...
contract EcoEarn is AccessControl {
    ...
    /**
     * @dev Transfers EcoEarn tokens from one address to another
     * @param from Address to send tokens from
     * @param to Address to send tokens to
     * @param amount Amount of tokens to transfer
     */
    function transferEcoEarnToken(address from, address to, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(amount > 0, "EcoEarn: Amount must be greater than 0");
        require(token.balanceOf(from) >= amount, "EcoEarn: Insufficient balance");

        // Transfer the tokens
        require(token.transferFrom(from, to, amount), "EcoEarn: Transfer failed");
